...
After bundle, profile, and home patches are composed, the launcher verifies the expected official row identities. Its final advanced overlay disables the official `ui-layout` row and explicitly keeps `ui-conversation` enabled. It does not overwrite the composed `ui-sidebar` enabled state: the default profile keeps the official occupant, while a profile may disable that row and supply a complete replacement through the public `sidebar` slot. Compatibility keeps all three official rows enabled.
...
The Client registers the `root` occupant and declares child seats for `sidebar`, `conversation`, `details`, and additive `shell.overlay` entries. The official `ui-sidebar` remains the default sidebar occupant and keeps ownership of its workspace, settings, and additive footer-action seats. A profile may instead disable that row and provide a replacement occupant that declares the complete child-seat set. The unchanged `ui-conversation` plugin continues to own the conversation and details surfaces. Third-party features can contribute to the same documented seats as in compatibility mode.
...
On macOS the advanced `BrowserWindow` uses `titleBarStyle: hiddenInset`, positioned traffic lights, a transparent background, `vibrancy: sidebar`, and `visualEffectState: followWindow`. The renderer keeps a transparent sidebar surface over the native vibrancy and adds a traffic-light inset around the official sidebar. Its 90 CSS-pixel collapsed column centers the official 56-pixel rail. The sidebar surface is non-draggable, while a desktop-owned strip above its content and to the right of the traffic lights provides a 32 CSS-pixel window drag target. A separate opaque caption row reserves the same 32 CSS pixels above the conversation and details columns. Matching the content inset to the drag region keeps feature-owned Header nodes outside the overlay without inspecting or rearranging them. Semantic controls and explicit no-drag contributions remain interactive.
