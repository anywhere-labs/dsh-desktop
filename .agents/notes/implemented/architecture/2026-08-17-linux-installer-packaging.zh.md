...
- **`toolsets.appimage: "1.0.3"`。** Electron Builder 26.15.3 默认的 AppImage 路径会下载 FUSE2 静态运行时，其 AppRun 会无条件注入 `--no-sandbox`，并要求用户在不再预装 `libfuse2` 的较新发行版上自行安装它。设置 `toolsets.appimage: "1.0.3"` 会改选静态（非 FUSE2）运行时，与 Electron Builder v27 的默认方向一致。该版本的 schema 只接受 `"0.0.0" | "1.0.2" | "1.0.3" | null`；部分 Electron Builder v27 文档提到的 `"1.1.0"` 在 26.15.3 上并非合法值，打包开始前就会被 Ajv 校验拒绝。
