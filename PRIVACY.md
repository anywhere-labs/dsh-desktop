...
| System notifications | Turn, job, approval, and question notices use generic titles and copy without session names, user text, or error details by default. When content previews are enabled (default for new installs; off for upgrades), notices also use the session title and preview the answer, question text, or approval reason. Disable content previews under Desktop settings to restore fully generic notices. Update notices include the available version. The operating system handles them locally without a DSH Desktop remote-push service. Notification history or cross-device synchronization depends on your system account settings. |
