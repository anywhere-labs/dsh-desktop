...
| 系统通知 | 回合、任务、审批与提问通知默认使用泛化标题与文案，不含会话名、用户文本或错误详情。开启内容预览时（新装机默认开启，升级用户默认关闭），通知还会使用会话标题并预览回答、问题文本或审批原因。在桌面设置中关闭内容预览可恢复完全泛化的通知。更新通知包含可用版本号。通知由本机操作系统处理，不经过 DSH Desktop 远程推送服务；操作系统的通知历史或跨设备同步取决于您的系统账户设置。 |
