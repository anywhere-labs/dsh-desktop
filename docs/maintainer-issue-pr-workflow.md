...
- 需要双语时，英文与中文使用独立段落或独立小节；不要用 `/` 把两种语言拼在同一行。
...
- “参考回复格式”“整理文案”或“提供模板”只授权草拟；只有用户明确指定要发布到某个 Issue/PR 时才可以评论。授权提交 PR 不连带授权回复 Issue。
- 从当前 package manifest 版本构建的产物只能称为“本地测试产物”。写“正式发布”“发布版”或“已发布”前，必须核对实时 tag、GitHub Release、官方资产及其来源。
