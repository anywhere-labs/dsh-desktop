...
内置插件市场已经提供发现、受管安装和管理界面。符合共同约定的插件更容易被目录来源准确描述和判断兼容性。我们希望让"按规范开发"成为对每个作者都有利的选择，而不是额外的负担。
...
[DSH Community Market](../dsh-community-market/README.zh.md) 已作为 Desktop 的私有内置 package 交付，提供插件发现、受管安装、卸载和启用状态管理。目录收录只代表来源提供了符合合同的条目；可安装性还要经过 Host 复核，二者都不等于安全审核或推荐。
