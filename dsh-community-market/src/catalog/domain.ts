...
/** Sole key of the `npm_registry` table. */
export const MARKET_NPM_REGISTRY_KEY = 'current'

/** Optional npm origin used for package-manager installs. Empty keeps the official registry. */
export interface MarketNpmRegistryRecord {
...
const npmRegistryValueSchema = npmRegistrySchema as unknown as ZodType<MarketNpmRegistryRecord>
const globalValueSchema = globalSchema as unknown as ZodType<MarketDomainGlobal>

/**
 * Market's durable state.
 *
 * Payloads are tables rather than the global slot, and the layout is
 ...
 * - The npm install origin is its own table. Source rewrites and cache writes
 *   must not clobber it, and an empty record means the official registry.
 *
...
export const marketDomainSpec = defineDomain({
  ...
  tables: {
    ...
    npm_registry: domainTable<typeof MARKET_NPM_REGISTRY_KEY, MarketNpmRegistryRecord>(npmRegistryValueSchema),
  },
})
...
export class DomainMarketStateStore implements MarketStateStore {
  ...
  constructor(private readonly domain: MarketDomain) {
    ...
    this.npmRegistry = domain.table('npm_registry')
  }
  ...
  getNpmRegistry(): string {
    return this.npmRegistry.get(MARKET_NPM_REGISTRY_KEY)?.origin ?? ''
  }

  async setNpmRegistry(origin: string): Promise<void> {
    await this.npmRegistry.put(MARKET_NPM_REGISTRY_KEY, { origin })
  }
...
export async function importLegacyMarketSettings(ctx: Context, store: DomainMarketStateStore): Promise<void> {
  ...
  if (legacy !== undefined && legacy.records.length > 0 && store.getSources().length === 0) {
  ...
  if (legacy?.npmRegistry !== undefined && legacy.npmRegistry !== '' && store.getNpmRegistry() === '') {
    await store.setNpmRegistry(legacy.npmRegistry)
    ctx.logger.info(
      `dsh-community-market: migrated the npm registry origin from ${legacy.from} into durable storage`,
    )
  }
