...
describe('community market durable state', () => {
  ...
  it('restores an optional npm registry origin in a new Host context', async () => {
    const medium = new MemoryMedium()
    const first = await bootMarketState(medium)
    await first.state.setNpmRegistry('https://registry.npmmirror.com')
    await first.dispose()

    const second = await bootMarketState(medium)
    expect(second.state.getNpmRegistry()).toBe('https://registry.npmmirror.com')
  })

  it('keeps an empty npm registry on the official origin', async () => {
    const boot = await bootMarketState(new MemoryMedium())
    expect(boot.state.getNpmRegistry()).toBe('')
  })

  it('flushes an npm registry chosen before durable storage exists', async () => {
    const deferred = new DeferredMarketStateStore()
    await deferred.setNpmRegistry('https://registry.npmmirror.com')
    const durable = new MemoryMarketStateStore()
    await deferred.adopt(durable)
    expect(durable.getNpmRegistry()).toBe('https://registry.npmmirror.com')
    expect(deferred.getNpmRegistry()).toBe('https://registry.npmmirror.com')
  })

  it('keeps a durable npm registry when storage already has one', async () => {
    const deferred = new DeferredMarketStateStore()
    await deferred.setNpmRegistry('https://registry.npmmirror.com')
    const durable = new MemoryMarketStateStore({ npmRegistry: 'https://registry.example.com' })
    await deferred.adopt(durable)
    expect(deferred.getNpmRegistry()).toBe('https://registry.example.com')
  })
  ...
  it('imports a pre-0.1.7 npm registry origin exactly once', async () => {
    const home = await temporaryHome({
      'dsh-community-market': { sources: [source], npmRegistry: 'https://registry.npmmirror.com' },
    })
    const medium = new MemoryMedium()

    const first = await bootMarketState(medium, home)
    expect(first.state.getSources()).toEqual([source])
    expect(first.state.getNpmRegistry()).toBe('https://registry.npmmirror.com')
    await first.state.setNpmRegistry('')
    await first.dispose()

    const second = await bootMarketState(medium, home)
    expect(second.state.getNpmRegistry()).toBe('')
  })
