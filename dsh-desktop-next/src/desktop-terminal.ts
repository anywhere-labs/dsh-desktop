...
function assertScriptValue(label: string, value: string): void {
  if (value.length === 0) throw new Error(`dsh-desktop-next: terminal ${label} must not be empty`)
  if (/[\0\r\n]/.test(value)) {
    throw new Error(`dsh-desktop-next: terminal ${label} must not contain NUL or newlines`)
  }
}
...
function escapeBatchText(value: string): string {
  if (/\r|\n/.test(value)) {
    throw new Error('dsh-desktop-next: terminal Windows welcome text must not contain newlines')
  }
  return value
    .replaceAll('^', '^^')
    .replaceAll('%', '%%')
...
function prepareStateDirectory(stateDir: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`dsh-desktop-next: terminal state path is not a private directory: ${stateDir}`)
  }
...
function prepareDesktopTerminalFiles(options: DesktopTerminalOptions): DesktopTerminalFiles {
  if (options.platform !== 'darwin' && options.platform !== 'win32') {
    throw new Error(`dsh-desktop-next: terminal is unsupported on ${options.platform}`)
  }
...
function resolveWindowsShell(
  ...
  throw new Error('dsh-desktop-next: terminal requires pwsh.exe, powershell.exe, or cmd.exe on Windows')
}
...
function windowsShellArgv(
  ...
  if (shell.kind === 'cmd') {
    ...
    if (welcome === undefined) {
      throw new Error('dsh-desktop-next: terminal did not generate the Windows command prompt welcome script')
    }
...
function resolveWindowsCommandProcessor(
  ...
  if (executable === undefined) {
    throw new Error('dsh-desktop-next: terminal requires cmd.exe to create a visible Windows console')
  }
...
function reportLaunchError(options: DesktopTerminalOptions, cause: Error): void {
  if (options.onLaunchError !== undefined) {
    try {
      ...
      process.stderr.write(`dsh-desktop-next: terminal error reporter failed: ${detail}\n`)
      return
    }
  }
  process.stderr.write(`dsh-desktop-next: failed to open terminal: ${cause.message}\n`)
}
