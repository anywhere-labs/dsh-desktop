import { z } from 'zod';
export const PLATFORM_CONNECTOR_VERSION = 'commerce.connector.v1';
export const SANDBOX_EVENT_VERSION = 'commerce.sandbox.event.v1';
...
export const platformConnectorCatalog = [
    {
...
export function createSandboxActionProposal(options) {
    const capability = findPlatformContract(options.platformId).capabilities.find(item => item.capability === options.capability);
    if (!capability)
        throw new Error(`sandbox_capability_not_found:${options.platformId}:${options.capability}`);
    return {
...
export function findPlatformContract(platformId) {
    const contract = platformConnectorCatalog.find(item => item.platformId === platformId);
    if (!contract)
        throw new Error(`sandbox_platform_not_found:${platformId}`);
    return contract;
}
