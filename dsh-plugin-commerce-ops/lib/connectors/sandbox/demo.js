...
import { createSandboxActionProposal, SANDBOX_EVENT_VERSION } from './contracts.js';
import { PlatformSandboxGateway } from './gateway.js';
...
export async function runPlatformSandboxDemo(platformId, scenario = 'success', environment = 'demo') {
    ...
    if (scenario !== 'credential_missing') {
        ...
        if (scenario === 'approval_rejected') {
            if (approval.status === 'pending')
                approval = approvals.reject(approval.approvalId, OWNER);
        }
...
async function append(ledger, input) {
