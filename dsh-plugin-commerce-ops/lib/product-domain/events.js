...
export const COMMERCE_EVENT_VERSION = 'commerce.event.v1';
...
export function parseCommerceEvent(input) {
    const event = businessEventSchema.parse(input);
    if (!commerceEventTypes.includes(event.eventType))
        throw new Error(`unsupported_commerce_event:${event.eventType}`);
    if (event.schemaVersion !== COMMERCE_EVENT_VERSION)
        throw new Error(`unsupported_commerce_event_schema:${event.schemaVersion}`);
    commerceEventPayloadSchemas[event.eventType].parse(event.payload);
    return event;
}
