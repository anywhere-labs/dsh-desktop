...
export async function runCompetitorPriceChangeSimulation(): Promise<SimulationResult> {
  const ledger = new EventLedger()
  const cases = new CaseStore()
  const router = new EventRouter(ledger)
  const trigger: BusinessEvent = {
...
export async function runApprovedCompetitorPriceChangeSimulation(): Promise<CompletedSimulationResult> {
  const ledger = new EventLedger()
  const cases = new CaseStore()
  const router = new EventRouter(ledger)
  ...
  const action: ActionProposal = { ...actionWithoutApproval, approvalId: approval.approvalId }
  await ledger.append(deriveEvent(trigger, {
  ...
  const record = cases.require(trigger.correlationId)
  if (record.status !== 'CLOSED' || record.responsibilityStatus !== 'RELEASED') throw new Error('simulation did not close the case')
...
function deriveEvent(parent: BusinessEvent, overrides: Pick<BusinessEvent, 'eventId' | 'eventType' | 'causationId' | 'subject' | 'payload'>): BusinessEvent {
  return {
    ...parent,
    ...overrides,
