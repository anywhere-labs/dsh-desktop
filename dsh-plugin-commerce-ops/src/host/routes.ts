...
export function registerCommerceOpsRoutes(webServer: CommerceWebServer, service = new CommerceOpsService()): () => void {
  ...
  const registrations = [
    webServer.register({ kind: 'exact', path: `${api}/runtime`, handler: (req, res) => {
      if (req.method !== 'GET' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'runtime requires local same-origin GET' })
      sendJson(res, 200, service.runtime)
    }}),
    webServer.register({ kind: 'exact', path: `${api}/inventory-alert/demo`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'inventory alert demo requires local same-origin POST' })
      try {
      ...
      } catch (cause) { return sendJson(res, 400, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/platform-sandbox/demo`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'platform sandbox demo requires local same-origin POST' })
      try {
        const body = await readJson(req)
        const platformId = body.platformId === 'xiaohongshu' ? 'xiaohongshu' : body.platformId === 'douyin' ? 'douyin' : undefined
        if (!platformId) return sendJson(res, 400, { error: 'platformId must be douyin or xiaohongshu' })
    ...
    webServer.register({ kind: 'exact', path: `${api}/voc/demo`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'voc demo requires local same-origin POST' })
      try {
        const body = await readJson(req)
    ...
    webServer.register({ kind: 'exact', path: `${api}/agent-space`, handler: async (req, res) => {
      if (!sameOrigin(req, origin)) return sendJson(res, 403, { error: 'agent space requires local same-origin access' })
      const url = new URL(req.url ?? `${api}/agent-space`, origin)
      if (req.method === 'GET') return sendJson(res, 200, service.getAgentSpaceSnapshot(url.searchParams.get('tenantId') ?? undefined))
      if (req.method === 'POST' && url.pathname === `${api}/agent-space/demo`) {
        try { return sendJson(res, 200, await service.seedAgentSpaceDemo()) }
        catch (cause) { return sendJson(res, 500, { error: cause instanceof Error ? cause.message : String(cause) }) }
      }
    ...
    webServer.register({ kind: 'exact', path: `${api}/agent-space/board`, handler: async (req, res) => {
      if (req.method !== 'GET' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'agent space board requires local same-origin GET' })
      const url = new URL(req.url ?? `${api}/agent-space/board`, origin)
      const actorId = url.searchParams.get('actorId') ?? 'local-user'
      try { return sendJson(res, 200, service.getHumanBoard(actorId)) }
      catch (cause) { return sendJson(res, 500, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'prefix', path: `${api}/agent-space/responsibilities/`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'responsibility action requires local same-origin POST' })
      ...
      if (!match) return sendJson(res, 404, { error: 'responsibility action not found' })
      try {
        const body = await readJson(req)
      ...
      } catch (cause) { return sendJson(res, 409, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/agent-space/demo`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'agent space demo requires local same-origin POST' })
      try { return sendJson(res, 200, await service.seedAgentSpaceDemo()) }
      catch (cause) { return sendJson(res, 500, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/decisions`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'decisions require local same-origin POST' })
      try { const body = await readJson(req); return sendJson(res, 201, service.recordHumanDecision({ insightId: String(body.insightId), decision: body.decision as 'accept' | 'modify' | 'reject', decidedBy: String(body.decidedBy ?? 'local-user'), ...(typeof body.note === 'string' ? { note: body.note } : {}) })) }
      catch (cause) { return sendJson(res, 400, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/responsibilities/transfer`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'responsibility transfer requires local same-origin POST' })
      try { const body = await readJson(req); return sendJson(res, 200, service.transferResponsibility(String(body.caseId), String(body.fromUserId), String(body.toUserId))) }
      catch (cause) { return sendJson(res, 403, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/sla/start`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'sla start requires local same-origin POST' })
      try { const body = await readJson(req); return sendJson(res, 201, service.startSla(String(body.caseId), String(body.ownerId), Number(body.durationMs))) }
      catch (cause) { return sendJson(res, 400, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/sla/evaluate`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'sla evaluation requires local same-origin POST' })
    ...
    webServer.register({ kind: 'exact', path: `${api}/notifications`, handler: (req, res) => {
      if (req.method !== 'GET' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'notifications require local same-origin GET' })
    ...
    webServer.register({ kind: 'exact', path: `${api}/metrics`, handler: async (req, res) => {
      if (req.method !== 'GET' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'metrics requires local same-origin GET' })
    ...
    webServer.register({ kind: 'exact', path: `${api}/daily-report`, handler: async (req, res) => {
      if (req.method !== 'GET' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'daily report requires local same-origin GET' })
    ...
    webServer.register({ kind: 'exact', path: `${api}/dry-run`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'dry run requires local same-origin POST' })
      try { return sendJson(res, 200, service.dryRun(await readJson(req))) }
      catch (cause) { return sendJson(res, 400, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'exact', path: `${api}/approvals`, handler: async (req, res) => {
      if (!sameOrigin(req, origin)) return sendJson(res, 403, { error: 'approvals require local same-origin access' })
      if (req.method === 'GET') return sendJson(res, 200, { approvals: service.listApprovals() })
      if (req.method !== 'POST') return sendJson(res, 405, { error: 'approvals support GET or POST' })
      try {
        const body = await readJson(req)
        const action = body.action
        const requesterId = typeof body.requesterId === 'string' ? body.requesterId : 'local-user'
        if (typeof action !== 'object' || action === null || Array.isArray(action)) return sendJson(res, 400, { error: 'action is required' })
        return sendJson(res, 201, service.createApproval(action as Parameters<CommerceOpsService['createApproval']>[0], requesterId))
      } catch (cause) { return sendJson(res, 400, { error: cause instanceof Error ? cause.message : String(cause) }) }
    }}),
    webServer.register({ kind: 'prefix', path: `${api}/approvals/`, handler: async (req, res) => {
      if (req.method !== 'POST' || !sameOrigin(req, origin)) return sendJson(res, 405, { error: 'approval action requires local same-origin POST' })
      const match = new URL(req.url ?? '/', origin).pathname.match(new RegExp(`^${api}/approvals/([^/]+)/(approve|reject)$`))
      if (!match) return sendJson(res, 404, { error: 'approval action not found' })
      try {
        const body = await readJson(req)
...
const WORKBENCH_CONTENT_TYPES: Readonly<Record<string, string>> = {
...
async function serveWorkbenchFile(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method !== 'GET' && req.method !== 'HEAD') return sendJson(res, 405, { error: 'commerce ops workbench requires GET' })
  const pathname = new URL(req.url ?? '/', 'http://localhost').pathname
  const suffix = pathname === WORKBENCH_PAGE || pathname === `${WORKBENCH_PAGE}/` ? 'index.html' : pathname.slice(`${WORKBENCH_PAGE}/`.length)
  let decoded = ''
  try { decoded = decodeURIComponent(suffix) } catch { return sendJson(res, 400, { error: 'invalid commerce ops workbench asset path' }) }
  if (!decoded || decoded.includes('\u0000') || decoded.includes('..') || decoded.includes('\\')) return sendJson(res, 400, { error: 'invalid commerce ops workbench asset path' })
  const root = resolve(workbenchDistRoot())
  const filename = resolve(root, decoded)
  const relativeName = relative(root, filename)
  if (relativeName.startsWith('..') || relativeName.includes('\\')) return sendJson(res, 400, { error: 'invalid commerce ops workbench asset path' })
  try {
  ...
  } catch { sendJson(res, 404, { error: 'commerce ops workbench asset not found' }) }
}

export function registerCommerceOpsWorkbenchPage(webServer: CommerceWebServer): () => void {
  const registrations = [
