...
export const inventoryAlertAtomDefinition: AgentAtomDefinition = {
...
export interface InventoryThresholdAtomInput {
  ...
  readonly inventory: { readonly skuId: string; readonly available: number; readonly reserved: number; readonly reorderPoint: number; readonly safetyStock: number; readonly evidenceRefs: readonly string[] }
  readonly sku: { readonly skuId: string; readonly productId: string; readonly title: string; readonly evidenceRefs: readonly string[] }
  readonly product: { readonly productId: string; readonly name: string; readonly evidenceRefs: readonly string[] }
}
...
export function runInventoryThresholdAtom(input: InventoryThresholdAtomInput): AgentAtomRunResult<InventoryThresholdAtomOutput> {
  const evidenceRefs = [...new Set([...input.inventory.evidenceRefs, ...input.sku.evidenceRefs, ...input.product.evidenceRefs])]
  if (input.inventory.evidenceRefs.length === 0) throw new Error('ATOM_MISSING_EVIDENCE')
  const values = [input.inventory.available, input.inventory.reserved, input.inventory.reorderPoint, input.inventory.safetyStock]
  if (values.some(value => !Number.isInteger(value) || value < 0)) throw new Error('ATOM_INVALID_INVENTORY')
  if (input.inventory.skuId !== input.sku.skuId || input.sku.productId !== input.product.productId) throw new Error('ATOM_INVALID_INVENTORY')
...
export function orchestrateProductEvent(event: Pick<BusinessEvent, 'eventType' | 'eventId'>, input: InventoryThresholdAtomInput): AgentAtomRunResult<InventoryThresholdAtomOutput> {
  if (event.eventType !== 'inventory.threshold.breached') throw new Error(`AGENT_UNSUPPORTED_EVENT:${event.eventType}`)
  if (event.eventId !== input.triggerEventId) throw new Error('AGENT_TRIGGER_MISMATCH')
