import { z } from 'zod'
import { runInventoryThresholdAtom, type InventoryThresholdAtomInput, type InventoryThresholdAtomOutput } from './atoms.js'
...
export const inventoryAlertSkillDefinition = {
  ...
  permissionScope: ['evidence.read', 'case.create', 'approval.request'] as const,
  allowedTools: [] as const,
  qaPolicy: 'required_evidence_and_deterministic_quantity' as const,
  approvalPolicy: 'L3_human_approval_required' as const,
  idempotencyKey: 'inventory-alert:{triggerEventId}:{skuId}',
  failureCodes: ['ATOM_MISSING_EVIDENCE', 'ATOM_INVALID_INVENTORY', 'ATOM_DUPLICATE_TRIGGER'] as const,
  acceptanceCriteria: ['输出建议数量', '输出 evidenceRefs', 'actionProposal.externalWrite=false'] as const,
...
export function runInventoryAlertSkill(input: InventoryAlertSkillInput): InventoryAlertSkillOutput {
  const result = runInventoryThresholdAtom(input as InventoryThresholdAtomInput)
  return inventoryAlertSkillOutputSchema.parse(result.output) as InventoryAlertSkillOutput
}

export function getProductSkill(skillId: string): typeof inventoryAlertSkillDefinition {
  if (skillId !== inventoryAlertSkillDefinition.skillId) throw new Error(`product_skill_not_found:${skillId}`)
