...
The Desktop browser is a real Chromium guest view owned by the desktop window's native layer, not a page rendered inside the Web carrier. The `desktop-browser` Host row (`dsh-plugin-desktop/browser`) consumes the shell's `desktopNativeBrowser` capability and adds the browsing semantics on top: one tab store per Session, the placement rules, the private panel channel, and the Agent tool. When no native browser capability exists, an ordinary Web boot or a headless Host registers no service, tool, or route, logs one informational line stating why, and installs the surface only if that capability appears later in the same generation.

Users open the surface from the conversation header's **Browser** icon control: a hover tooltip names it, its pressed state reports whether the panel is open, and its position stays fixed while the tab count grows. The Agent opens or hides the same panel for its Session through the `desktop_browser` tool's `panel` action, which the panel applies on its next exchange with the Host, and every other tool action reveals the panel on its own so the page the Agent is working on stays in sight. The panel carries a tab strip, back, forward, reload or stop, an address field, zoom presets from 50 to 150 percent, and a menu with the two logical layouts and the active tab's visit history. The `fit` layout scales the page into the panel rectangle; the `desktop` layout pins the logical viewport to 1280 CSS pixels wide and scales that page into the same rectangle. A status row reports the logical viewport size, the applied zoom, the layout, and whether the last Host exchange succeeded. One Session keeps at most 12 tabs, and opening another fails with `BROWSER_TAB_LIMIT`. The toolbar's remaining column control, **fill the workspace beside the sidebar**, hands the conversation column and the right column to the page while the left sidebar, the caption row and the window controls all stay in place, with the same button restoring the shared row; the width itself belongs to the frame's own divider handle. Closing the panel keeps its tabs, and the frame keeps the width for the next open.

An address typed without a scheme is completed the way a browser completes it: a bare host or a `host:port` pair opens over `https`, while `localhost`, `127.0.0.1`, `[::1]` and `*.localhost` open over `http` because a local server has no certificate. A named scheme is never rewritten, so the navigation policy still sees `file:`, `mailto:` and `javascript:` addresses as what they are. When a page cannot be opened, its address stays in the address field and the tab keeps that link, and the page area shows the panel's own notice with the reason, a retry, and a dismiss control, instead of falling back to a blank page or to the page the user was leaving. A refused navigation leaves the current document where it is and reports the refusal the same way.

Guest views present the browser they actually are: the User-Agent, `navigator.userAgentData` and the `sec-ch-ua*` request headers carry the Chromium version Electron runs and no application token, with `Accept-Language` following the panel's own language list.

The panel's interface ships inside the desktop Client module and follows the profile mode, so turning the `desktop-browser` row off removes the Host service, the private route and the Agent tool while the header control stays until the shell runs in `compatibility` mode.

Google account pages cannot be completed inside this window, so the panel hands that work to Chrome. A guest page that carries recent input — the user's own pointer or key activity, or an Agent click dispatched through the panel — may ask for Chrome when it needs a Google account: the sign-in control on Gemini, or a sign-in route on `accounts.google.com`. The panel opens stock Chrome, waits until Chrome itself has left Google's sign-in form, copies the resulting Google cookies into the shared guest profile, and reloads every open Google tab signed in. A page that reaches such an address on its own, with no recent input, opens nothing. The status row reports the phase, the tools menu starts or cancels the same login by hand, one page asks once, and a cancel stops the asking; a profile that already holds a live Google session never opens Chrome this way.

The toolbar's Chrome control hands the whole tab strip over: every open tab with an `http(s)` address becomes a tab of one new window in the user's own Chrome profile — the profile that already holds their sign-ins — which needs no debugging port and leaves the panel, its tabs and their history untouched. A Session whose tabs hold no such address is refused with `BROWSER_NO_URLS`.

The panel is ordinary renderer DOM, but the page is composited by the window server above it. The renderer therefore reports the placeholder rectangle in CSS pixels together with the zoom, layout, and visibility it wants, and the shell performs the placement, clamped inside the content area. Panel traffic uses the private same-origin path `/api/dsh-desktop-browser`; every request carries one Session id, and every action is a single JSON body. Every guest view renders in one shared persistent Chromium profile (`persist:dsh-desktop-browser-profile`), so signing in to a site in one conversation is the sign-in the next conversation opens it with, and cookies, storage and cache survive a restart; tabs, history and the panel column still belong to the Session they were opened in.

The panel draws the right track as its own layer above the shipped right Sidebar, which keeps that track's seat: the Sidebar's session surface stays mounted while the panel is open, so its own commands keep working — opening a file from the conversation or from the workspace file list previews there as usual. When the Sidebar raises its panel while the browser holds the column, the panel cedes the column and the Sidebar shows that content; the header control brings the panel back, and a panel opened over a Sidebar that is already showing stays where it is.

Other plugins use the Host service `ctx.desktopBrowser`, which exists whenever the native capability does. It exposes `version`, `available`, `open()`, `store()`, `existing()`, `state()`, `openTab()`, `closeTab()`, `act()`, `panel()`, `directive()`, and `subscribe()`; the [plugin service contract](docs/plugin-services.md) documents each member and its stability. The Agent tool is named `desktop_browser` and covers navigate, snapshot, screenshot, click, fill, press, scroll, console, evaluate, tabs, close, and panel. Page text returned to the Agent is untrusted task data, and screenshots require an image-capable model.
