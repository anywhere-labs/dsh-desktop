...
Users open the surface from the conversation header's **Browser** control, which also shows that Session's open tab count while it has tabs. The Agent opens or hides the same panel for its Session through the `desktop_browser` tool's `panel` action, which the panel applies on its next exchange with the Host, and every other tool action reveals the panel on its own so the page the Agent is working on stays in sight. The panel carries a tab strip, back, forward, reload or stop, an address field, zoom presets from 50 to 150 percent, and a menu with the two logical layouts and the active tab's visit history. The `fit` layout scales the page into the panel rectangle; the `desktop` layout pins the logical viewport to 1280 CSS pixels wide and scales that page into the same rectangle. A status row reports the logical viewport size, the applied zoom, the layout, and whether the last Host exchange succeeded. One Session keeps at most 12 tabs, and opening another fails with `BROWSER_TAB_LIMIT`. Beside the page controls, the toolbar carries controls for the column itself: **narrower** and **wider** move it by one eighth of the window width, clamped to the frame's own limits, and **fill the workspace beside the sidebar** hands the conversation column and the right column to the page while the left sidebar, the caption row and the window controls all stay in place, with the same button restoring the shared row. The frame keeps owning the drag handle, so a drag and these buttons act on one width. Closing the panel keeps its tabs, and the frame keeps the width for the next open.
