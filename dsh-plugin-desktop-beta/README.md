...
Electron ships no find UI of its own, so Ctrl+F and Cmd+F are dead keystrokes in a plain Electron shell. Desktop owns the gesture in every mode: it opens a frame-wide find bar that searches the rendered page, excludes its own surface, skips content Chromium does not render, and keeps upstream's folded turns (`hidden="until-found"`) searchable — navigating to a match inside one opens that turn through upstream's own `beforematch` reveal. Matches paint through the CSS Custom Highlight API rather than DOM wrappers, so the streaming transcript is never mutated. Enter and Shift+Enter, Ctrl/Cmd+G, and F3 step through matches with wrap-around; Escape closes the bar and drops its highlights; results past the 1000-match cap are reported as truncated.
