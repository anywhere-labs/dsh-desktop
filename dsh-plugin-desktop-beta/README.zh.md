...
打包后的可执行文件也接受一个目录。相对路径以发起进程的工作目录为基准；包含空格的路径须加引号，以短横线开头的目录名须放在 `--` 后。DSH 会等待选定的 Profile 和客户端就绪，再注册或复用工作区，并通过现有工作区流程打开其中的空白会话。应用已经运行时，由现有实例处理请求。无效路径或工作区操作失败会显示错误，不切换当前会话。Windows 仍执行现有工作区磁盘校验。

此功能不会安装全局命令、文件管理器右键菜单或 URL 协议。
