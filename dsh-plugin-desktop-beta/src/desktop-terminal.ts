...
function macPnpmShim(options: DesktopTerminalOptions): string {
  return [
    ...
    [
      ...
      `exec ${quoteSh(options.appExecutable)} ${quoteSh(options.pnpmBinPath)} ${PNPM_IGNORE_PM_ON_FAIL} ${PNPM_IGNORE_MINIMUM_RELEASE_AGE} "$@"`,
    ].join(' '),
    '',
  ].join('\n')
}
...
function windowsPnpmShim(): string {
  return [
    ...
    `"%${WINDOWS_APP_EXECUTABLE}%" "%${WINDOWS_PNPM_ENTRY}%" ${PNPM_IGNORE_PM_ON_FAIL} ${PNPM_IGNORE_MINIMUM_RELEASE_AGE} %*`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
