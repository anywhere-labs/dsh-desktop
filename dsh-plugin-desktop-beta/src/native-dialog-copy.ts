...
const COPY: Record<DesktopLocale, DesktopNativeCopy> = {
  ...
  ru: {
    ok: 'ОК',
    pluginRecoveryTitle: 'Не удалось загрузить плагины',
    pluginRecoveryMessage: 'Не все плагины удалось загрузить.',
    unknownPlugin: 'Неизвестный плагин клиента',
    missingPluginError: 'Загрузчик плагинов не предоставил сообщение об ошибке.',
    failedPlugins: 'Не удалось загрузить плагины:',
    pluginRecoveryInstructions: 'Обновите или удалите проблемные сторонние плагины через терминал DSH, затем перезапустите приложение.',
    openTerminal: 'Открыть терминал DSH',
    restart: 'Перезапустить DSH Desktop',
    dismiss: 'Закрыть',
    updateAvailableTitle: 'Доступно обновление DSH Desktop',
    updateAvailableMessage: version => `Доступна версия DSH Desktop ${version}.`,
    downloadUpdate: 'Загрузить обновление сейчас?',
    installStableAlongsideBeta: 'Установить стабильную версию рядом с DSH Desktop Beta? Бета-версия останется на компьютере.',
    download: 'Загрузить',
    later: 'Позже',
    updateCheckFailedTitle: 'Не удалось проверить обновления',
    updateCheckFailedMessage: 'Не удалось получить сведения об обновлениях.',
    tryAgainLater: 'Повторите попытку позже.',
    upToDateTitle: 'Установлена последняя версия DSH Desktop',
    upToDateMessage: 'Вы используете последнюю версию.',
    installedVersion: version => `Установленная версия: ${version}`,
    installerUnavailable: 'Эта версия приложения не поддерживает загрузку установщика.',
    updateDownloadedTitle: 'Обновление DSH Desktop загружено',
    updateReady: version => `Версия DSH Desktop ${version} готова к установке.`,
    macInstallInstructions: 'Образ диска открыт. Замените DSH Desktop в папке «Программы», затем откройте приложение снова.',
    windowsInstallQuestion: 'Перезапустить DSH Desktop и сразу запустить установщик?',
    restartAndInstall: 'Перезапустить и установить',
    saveInstallerTitle: 'Сохранение установщика обновления',
    ...
    updateInstalled: version => `Версия DSH Desktop ${version} установлена.`,
    removeInstallerQuestion: path => `Удалить загруженный установщик, чтобы освободить место на диске?\n\n${path}`,
    deleteInstaller: 'Удалить установщик',
    keepInstaller: 'Оставить установщик',
    terminalErrorTitle: 'Не удалось открыть терминал DSH',
    terminalErrorMessage: 'Не удалось запустить терминал. Повторите попытку.',
    diagnosticsErrorTitle: 'Не удалось экспортировать данные диагностики',
    diagnosticsErrorMessage: 'Не удалось создать архив диагностики. Повторите попытку.',
    skippedPluginTitle: 'Плагин интерфейса не загружен',
    skippedPluginBody: (name, additionalCount) => additionalCount > 0
      ? `Плагин интерфейса «${name}» не установлен в этом профиле. Других таких плагинов: ${additionalCount}.`
      : `Плагин интерфейса «${name}» не установлен в этом профиле.`,
    unsupportedStorageTitle: 'Хранилище может быть несовместимо',
    unsupportedStorageBody: label => `Путь «${label}» находится на томе, из-за которого могут возникать сбои при выполнении команд в изолированной среде или установке плагинов.`,
    profileCompatibilityTitle: 'Предупреждение о совместимости профиля',
    profileCompatibilityMessage: profileName =>
      `Версия Desktop, с которой в последний раз использовался профиль «${profileName}», отличается от текущей:`,
    profileCompatibilityDetail: (previousDesktopVersion, previousDshVersion, _currentProductName, currentDesktopVersion, currentDshVersion) =>
      `Предыдущие версии Desktop/DSH: ${previousDesktopVersion}/${previousDshVersion}\nТекущие версии Desktop/DSH: ${currentDesktopVersion}/${currentDshVersion}`,
    profileCompatibilityUnknownDetail: (_currentProductName, currentDesktopVersion, currentDshVersion) =>
      `Предыдущие версии Desktop/DSH: неизвестны/неизвестны\nТекущие версии Desktop/DSH: ${currentDesktopVersion}/${currentDshVersion}`,
    profileCompatibilityWarning: 'Различия между версиями DSH могут привести к следующим проблемам:\n1. Не удастся загрузить историю сессий.\n2. Некоторые плагины текущего профиля могут оказаться несовместимыми, вызывать ошибки или сбои приложения.\nРекомендуем перейти на совместимый профиль или создать новый.',
    switchProfile: 'Сменить профиль',
    useProfileAnyway: 'Всё равно использовать',
    sharedDataDirectoryTitle: 'Общий каталог данных',
    sharedDataDirectoryMessage: (currentProductName, otherProductName) =>
      `${currentProductName} использует один каталог данных вместе с ${otherProductName}.`,
    sharedDataDirectoryDetail: (currentDirectory, channelDirectory) =>
      `Текущий каталог данных:\n${currentDirectory}\n\nРекомендуемый каталог:\n${channelDirectory}`,
    sharedDataDirectoryWarning: (otherProductName, sessionsDirectory) =>
      'Эти версии работают с разными ядрами DSH. При общем каталоге данных установленные плагины могут периодически не загружаться.\n'
      + `Если перейти на отдельный каталог, текущий останется без изменений и ${otherProductName} продолжит им пользоваться.\n`
      + `Существующие сессии останутся в ${sessionsDirectory}. Чтобы вернуть к ним доступ, скопируйте эту папку в новый каталог данных.`,
    useChannelDataDirectory: 'Использовать отдельный каталог',
    shareDataDirectoryAnyway: 'Оставить общий каталог',
    sharedDataDirectoryFailed: 'Не удалось перейти на отдельный каталог данных. Текущий каталог не изменён; можно повторить попытку при следующем запуске.',
    quit: 'Выйти',
