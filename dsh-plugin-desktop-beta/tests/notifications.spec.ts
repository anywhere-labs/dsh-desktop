...
describe('desktop notifications Host plugin', () => {
  ...
  it('uses static titles when response previews are disabled and when a turn fails', async () => {
    ...
    expect(harness.notifyAttention.mock.calls).toEqual([
      [{ title: 'User Turn Completed', body: 'A user-initiated turn has finished.' }],
  ...
  it('applies the preview switch live and never carries answers into later turns or sessions', async () => {
    ...
    expect(harness.notifyAttention.mock.calls[0]![0]).toEqual({ title: 'User Turn Completed', body: 'A user-initiated turn has finished.' })
    for (const [notification] of harness.notifyAttention.mock.calls.slice(1)) {
  ...
  it('does not preview interrupted responses or failed turns', async () => {
    ...
    for (const kind of ['completed', 'error'] as const) {
      ...
      await harness.sessionEvent(active, { ...response, data: { ...response.data, interrupted: true } as Extract<SessionEvent, { type: 'assistant/message' }>['data'] })
      const reason = kind === 'error' ? { kind, error: { code: 'UNKNOWN' as const, message: 'Private error' } } : { kind }
      await harness.sessionEvent(active, event('turn/end', { turn: 1, reason }, 4))
    }
  ...
  it('treats max-tokens as a failure and keeps non-failure endings silent', async () => {
    ...
    expect(harness.notifyAttention).toHaveBeenCalledWith({
      title: 'Example session',
      body: 'The turn hit the context limit (max-tokens). Open DSH Desktop to continue.',
    })
  })

  it('falls back to the tool-call count when a turn produces no prose', async () => {
    const harness = createHarness(['sessions'])
    const active = session('direct')
    await harness.sessionEvent(active, event('turn/start', { turn: 1 }, 1))
    await harness.sessionEvent(active, userMessage('user', 2))
    await harness.sessionEvent(active, assistantMessage([{ type: 'tool-call', id: 'c1' as never, name: 'bash', arguments: '{}' }], 3))
    await harness.sessionEvent(active, assistantMessage([{ type: 'tool-call', id: 'c2' as never, name: 'edit', arguments: '{}' }, { type: 'tool-call', id: 'c3' as never, name: 'write', arguments: '{}' }], 4))
    await harness.sessionEvent(active, event('turn/end', { turn: 1, reason: { kind: 'completed' } }, 5))
    expect(harness.notifyAttention).toHaveBeenCalledExactlyOnceWith({
      title: 'Example session',
      body: 'Completed 3 tool calls.',
    })
  })
