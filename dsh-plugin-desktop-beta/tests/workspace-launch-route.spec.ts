...
import type { WorkspaceLaunchDelivery } from '../src/workspace-launch-contract.ts'

async function serve(run: (url: string, queue: WorkspaceLaunchQueue) => Promise<void>, delivery?: WorkspaceLaunchDelivery) {
  const queue = new WorkspaceLaunchQueue(async () => true, vi.fn(), vi.fn())
  const server = createServer((req, res) => { void handleWorkspaceLaunch(req, res, 'http://desktop.test', delivery ?? queue, req.url === '/complete') })
...
describe('private workspace delivery route', () => {
  ...
  it('waits for a remote acknowledgement and preserves a rejected ID as a conflict', async () => {
    await serve(async url => {
      const response = await fetch(`${url}/complete`, {
        method: 'POST', headers: { origin: 'http://desktop.test' }, body: JSON.stringify({ id: 'stale' }),
      })
      expect(response.status).toBe(409)
    }, { next: async () => null, complete: async () => false })
  })
  it('returns unavailable when the Host transport cannot deliver or acknowledge', async () => {
    const unavailable = async (): Promise<never> => { throw new Error('Host channel closed') }
    await serve(async url => {
      for (const path of ['', '/complete']) {
        const response = await fetch(`${url}${path}`, {
          method: 'POST', headers: { origin: 'http://desktop.test' }, body: JSON.stringify({ id: 'pending' }),
        })
        expect(response.status).toBe(503)
      }
    }, { next: unavailable, complete: unavailable })
  })
})
