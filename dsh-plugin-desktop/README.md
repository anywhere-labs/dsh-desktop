...
The top-level `build.files` excludes the arm64 build of every native optional dependency (`sharp`, `koffi`, `ripgrep`, `node-addon-require-builtin`, `@deepseek-ai/node-addon-landlock-run`), the Linux equivalent of what `build.mac.x64ArchFiles` does for the universal macOS build. `node_modules` carries both `linux-x64` and `linux-arm64` prebuilds for these packages regardless of build target — nothing in the plain `node-modules` linker or electron-builder's own file matching narrows that to the current target's architecture on its own — so without this exclusion list, the never-loaded arm64 binaries ship inside every Linux package for no reason. These patterns live in the shared top-level array rather than under `build.linux`: electron-builder's Linux packaging quietly falls back to a bare `**/*` sweep of the whole package directory (pulling in `src/`, `tests/`, `scripts/`, and every config file) the moment `build.linux.files` is a non-empty array, verified with electron-builder's own `builder-debug.yml` pattern dump — a platform override that should only ever narrow the file set instead widens it. The shared array carries no such risk and is a no-op on macOS and Windows, since those hosts never install a `linux-arm64` prebuild in the first place. Extend the same array, not `asarUnpack`, when a future native dependency adds its own per-architecture Linux prebuild — and never move these patterns into `build.linux.files`.
