...
Enhanced mode is an explicitly composed desktop presentation for macOS and Windows. After all user patches have been read, the launcher disables the official `ui-layout` Loader row, keeps `ui-conversation` enabled, preserves the profile-selected `ui-sidebar` state, and applies the selected mode to `desktop-shell`. The default profile therefore keeps the official sidebar, while a profile plugin can disable that row and provide a replacement through the same public `sidebar` slot.

The desktop Client provides the immutable `desktopWindow` native-geometry service in all presentation modes. Enhanced mode has its own Cordis effects, `layout` service, and `root` slot registration; it does not install the independent compatibility/extended frame. Its root declares seats for the profile-selected sidebar, conversation, details, and overlay contributions. The official sidebar remains the default `sidebar` occupant and continues to declare the workspace browser, settings shell, and additive footer-action seats unless a profile replaces that complete occupant and its child declarations. This preserves public slot composition while the desktop package owns only its internal-caption geometry and native material.
...
On macOS the enhanced window uses hidden-inset geometry with traffic lights at `x=16, y=16` and a 32 CSS-pixel content reservation matching the native drag region. Keeping the complete conversation and details surfaces below that opaque caption prevents the drag layer from covering their headers. Its 90 CSS-pixel collapsed column centers the official 56-pixel rail below the same reservation, while optional native `sidebar` vibrancy remains available. Buttons, links, inputs, editable fields, menus, tabs, switches, dialogs, and explicit `.dshDesktopNoDrag` contributions remain interactive through precise `app-region: no-drag` exclusions. On Windows the official sidebar keeps compatibility geometry: 56 pixels collapsed, 280 pixels by default when expanded, and the same upstream transition behavior, while its transparent surface reveals the selected supported material. The enhanced window keeps its 32 CSS-pixel internal caption row and native overlay controls; this geometry is independent from the 36-pixel compatibility/extended frame. Linux rejects enhanced mode rather than silently falling back to a presentation different from the persisted setting.
