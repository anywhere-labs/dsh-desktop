...
The launcher-managed `desktop` profile also composes the pinned `dsh-llm-guardian` Host and Web Client bundle. It adds Provider-card health checks, account-usage summaries, per-model local statistics, refresh and timeout controls, and an optional local token ceiling through ordinary DSH services and settings. The integration does not modify the pinned `deepseek-harness` submodule or expose Electron APIs to the renderer; see [Provider health and model usage](../docs/provider-usage.en.md) for screenshots, data boundaries, and implementation details.
