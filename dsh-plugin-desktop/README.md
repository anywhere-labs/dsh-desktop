...
When the Desktop window is unfocused, a permission request that stays pending past the current event-loop turn raises a native approval notification naming only the tool awaiting a decision (and the asker's reason when content previews are on); immediately decided, cancelled, or auto-answered requests stay silent, and at most one approval notification per session is pending at a time. A model question waiting for your answer raises a native notification with the session name and question preview. Each question group notifies once, without waiting for the turn to end; cancelled requests and requests answered immediately stay silent. A direct user turn that reaches `completed` raises a native completion notification; `error` and `max-tokens` endings raise a needs-attention notification. Completed and failed background jobs use the same native attention path. Turn-completion and failure alerts stay silent for aborted, blocked, interrupted, plugin-initiated, continuation-only, mismatched, and subagent turns; killed background jobs also stay silent. Clicking a notification reveals and focuses the window. macOS and Linux increment the application badge, while Windows flashes the taskbar button; showing, focusing, or releasing the window clears that attention. The live `dsh-desktop-notifications` settings namespace provides independent `notifyOnApprovalRequest`, `notifyOnUserQuestion`, `notifyOnTurnCompletion`, `notifyOnTurnFailure`, `notifyOnJobCompletion`, and `notifyOnJobFailure` switches, all enabled by default. Pending questions and completed or failed user turns use the latest session title as the notification heading, including restored titles and manual renames; before a title is saved, the heading follows the session list: project directory name, then session ID. Completed user turns also preview the final answer by default: plain text is limited to 400 visible characters, with an ellipsis for longer answers, and the operating system may display fewer lines. Reasoning blocks, tool calls, interrupted responses, and earlier turns are excluded. Question previews use only the question text, omitting supporting details and option descriptions, and have the same 400-character limit. Turn off **Show content previews in notifications** (`showResponsePreview`) under **Desktop settings → Desktop notifications**, or in Setup, to hide question and answer text while keeping the session title and a brief status message. **Model needs your answer** controls question notifications independently of turn completion. Missing or empty final answers, failures, and background jobs keep generic text. Existing settings and Profile preferences default missing preview and question-notification switches to enabled; Safe Mode disables both.
