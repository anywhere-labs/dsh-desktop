...
The Desktop settings page provides an explicit, interface-independent agent response-language preference: Automatic, Chinese, or English. Automatic is the default and adds no language constraint. Selecting Chinese or English projects a dynamic system-prompt section into main-agent and delegated-subagent assemblies; the preference affects subsequent model steps immediately without restarting the application or recreating a session. An explicit language request in the current conversation remains authoritative, while code, identifiers, commands, paths, and direct quotations retain their original text.
