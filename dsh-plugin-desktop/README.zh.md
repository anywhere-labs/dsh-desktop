...
`desktop-pnpm` Host row 会提供 `ctx.desktopPnpm`，用于针对不可变激活 profile 执行受管 package operation。`run(args, signal?)` 会在激活 profile 目录中直接执行内置 pnpm；它是低层 operation，不承诺 DSH profile 初始化、调用方相对 source 锚定或 bundle reconcile。`runPlugin(args, invokingDir, signal?)` 会为 remove、update 和 dependency repair 启动内置 `dsh plugin --profile <active>`，但拒绝 `add`。插件安装使用 `installPlugin(request)`：它会把 receipt 绑定到默认的精确 npm `name@version` 目标，或调用方显式提供的结构化 source，随后在 spawn 前快照 profile，并在完成前封存或恢复快照。Git source 仍被限制为固定 commit 的 GitHub 目标，可选一个规范化 subpath；service 依旧不接受任意 shell 文本或自由拼接的 package-manager target。上游 CLI 仍拥有 pnpm profile working directory 和 bundle reconcile 语义。
