...
Desktop 设置页提供独立于界面语言的显式代理回复语言偏好：自动、中文或英文。默认的“自动”不添加语言约束；选择中文或英文后，Host 会把动态 system-prompt section 同时投影到主代理和所有委派子代理。偏好会立即影响后续模型步骤，无需重启应用或重新创建会话。当前会话中的明确语言要求始终优先；代码、标识符、命令、路径和直接引用保留原文。
