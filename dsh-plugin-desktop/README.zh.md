...
`dist:win` 构建的 NSIS 安装器使用 Electron Builder 的 `store` 压缩模式。这样会增大 Setup 下载体积，但能避免安装时解码压缩载荷，在继续使用原有 NSIS 路径（`useZip=false`）的前提下优先提高安装速度。绿色 ZIP 仍使用 Electron Builder 的默认压缩。
