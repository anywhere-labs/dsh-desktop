...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Aera-Code-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Aera Code.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Aera Code.exe: ${portablePath}`)
  }
