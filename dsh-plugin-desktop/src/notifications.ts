...
interface NotificationCopy {
  ...
  'approval-requested': (toolName: string) => Pick<DesktopNotification, 'body'>
...
const NOTIFICATION_COPY: Record<DesktopLocale, NotificationCopy> = {
  en: {
    ...
    'approval-requested': (toolName: string) => ({ body: `"${toolName}" is waiting for your approval.` }),
  ...
  zh: {
    ...
    'approval-requested': (toolName: string) => ({ body: `「${toolName}」正在等待你的批准。` }),
    ...
    sessionsCtx.effect(() => {
      ...
      const stopApprovals = sessionsCtx.on('approval/request', async (request, next) => {
        ...
        const timer = setTimeout(() => {
          ...
          delivered = true
          const notification = NOTIFICATION_COPY[sessionsCtx.desktopRuntime.locale]['approval-requested'](request.toolName)
          try {
            sessionsCtx.desktopRuntime.notifyAttention({
              title: notificationTitle(session),
              body: settings.showResponsePreview && request.reason
                ? plainTextPreview(`${notification.body} ${request.reason}`)
                : notification.body,
            })
          } catch (error) {
