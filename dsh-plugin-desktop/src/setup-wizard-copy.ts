/** Desktop-owned copy for the pre-Host native Setup Wizard. */
...
const COPY: Record<DesktopLocale, DesktopSetupWizardCopy> = {
  ...
  ru: {
    ...
    aaNextTitle: 'Подключите устройство',
    aaNextBody: 'После настройки откройте «Подключение телефона» на боковой панели, чтобы продолжить подключение.',
    aaNextDesktop: 'Если на этом компьютере установлено приложение Agents-Anywhere, управляйте подключением в нём.',

    beta: 'Бета',
    title: 'Настройка DSH Desktop',
    profile: 'Профиль',
    welcomeTitle: 'Добро пожаловать в DSH Desktop',
    welcomeBody: 'Настройте вид окна, подключение телефона и уведомления для текущего профиля.',
    firstProfileSetup: 'Перед первым использованием этого профиля завершите настройку DSH Desktop.',
    startSetup: 'Начать настройку',
    presentationTitle: 'Выберите режим окна',
    presentationBody: 'Выберите расположение элементов и способ управления окном.',
    ...
    unavailableOnLinux: 'Сейчас этот режим доступен на macOS и Windows.',
    windowMaterial: 'Выберите оформление окна',
    windowMaterialBody: 'Выберите эффект фона окна.',
    materialOff: 'Сплошной фон',
    materialOffBody: 'Непрозрачный однотонный фон окна.',
    materialTransparent: 'Стеклянный фон',
    materialTransparentBody: 'Размытое изображение того, что находится за окном.',
    materialMica: 'Mica',
    materialMicaBody: 'Системный эффект Mica, если он поддерживается Windows.',
    browserTitle: 'Настройка доступа через браузер',
    browserBody: 'Разрешите открывать текущий профиль в браузере и выберите, с каких устройств он будет доступен.',
    openBrowser: 'Разрешить открывать профиль в браузере',
    browserCompatibilityNotice: 'Доступ через браузер работает только в режиме совместимости.',
    browserCompatibilityDialogTitle: 'Перейти в режим совместимости?',
    browserCompatibilityDialogBody: 'Для доступа к этому профилю через браузер нужен режим совместимости. Продолжить, изменить режим окна и включить доступ через браузер?',
    confirmBrowserCompatibility: 'Переключить и включить',
    cancelBrowserCompatibility: 'Отмена',
    networkExposure: 'Доступ по сети',
    networkExposureBody: 'Разрешите доступ только с этого компьютера или также с других устройств в локальной сети через HTTPS.',
    loopback: 'Только этот компьютер',
    loopbackBody: 'Подключаться смогут только браузеры на этом компьютере.',
    lan: 'Локальная сеть',
    lanBody: 'Другие устройства в той же локальной сети смогут подключаться по ссылке. Сначала установите на каждом из них сертификат этого компьютера и отметьте его как доверенный.',
    ...
    confirmLan: 'Включить доступ по локальной сети',
    cancelLan: 'Оставить доступ только с этого компьютера',
    marketTitle: 'Выберите каталог плагинов',
    marketBody: 'Выберите каталог плагинов для текущего профиля. Одновременно может работать только один.',
    marketDisabled: 'Отключить каталог плагинов',
    marketDisabledBody: 'Не загружать интерфейс каталога плагинов.',
    communityMarket: 'dsh-community-market',
    communityMarketBody: 'Открытый каталог, встроенный в DSH Desktop. Поддерживает собственные источники данных.',
    dshMarket: 'dsh-market',
    dshMarketBody: 'Популярный каталог сообщества на основе данных awesome-dsh-plugin.',
    notificationsTitle: 'Настройка уведомлений',
    notificationsBody: 'Получайте системные уведомления о завершении задач и ошибках. Содержимое переписки в них не показывается.',
    ...
    skipDialogTitle: 'Пропустить настройку?',
    skipDialogBody: 'Позже эти параметры можно изменить в разделе «Настройки» → «Настройки Desktop».',
    cancelSkip: 'Продолжить настройку',
    confirmSkip: 'Пропустить',
    successTitle: 'Настройка завершена',
    successBody: 'Настройки Desktop для текущего профиля сохранены.',
    startUsing: 'Открыть DSH Desktop',
    invalidState: 'Не удалось загрузить данные настройки. Закройте это окно и попробуйте ещё раз.',
  },
}
