...
describe('desktop Host pnpm runtime', () => {
  it.each(['darwin', 'linux'] as const)('creates a pnpm-only public PATH on %s', (platform) => {
    ...
    expect(pnpm).toContain('pnpm_config_pm_on_fail=ignore')
    expect(pnpm).toContain('pnpm_config_manage_package_manager_versions=false')
    expect(pnpm.match(/--config\.minimumReleaseAge=0/gu)).toHaveLength(1)
    expect(pnpm.match(/--pm-on-fail=ignore/gu)).toHaveLength(1)
  ...
  it('creates Windows batch shims without publishing the private Node directory', () => {
    ...
    expect(pnpm).toContain('set "pnpm_config_pm_on_fail=ignore"')
    expect(pnpm).toContain('set "pnpm_config_manage_package_manager_versions=false"')
