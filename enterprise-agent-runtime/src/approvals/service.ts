...
export class ApprovalService {
  ...
  create(action: ApprovalAction, requesterId: string, requestedApprovalId?: string): ApprovalRecord {
    const approvalId = requestedApprovalId ?? `approval_${Date.now()}_${this.records.size + 1}`
...
function caseIdFromAction(action: ApprovalAction): string | undefined {
  const value = action.payload.caseId
